<script>
  ...
  let currentUser = 0;
  let maxDisplayedUsers = 2;
  let usersLoaded = false;

  const getUsers = async () => {
    await fetch(
      `/user?user_id=${currentUser}&user_offset=${maxDisplayedUsers+currentUser}`,
      {
        method: "GET",
      }
    ).then((response) => {
...
<Page>
  ...
  {#if users.length > maxDisplayedUsers && currentUser == 0}
    <div class="list-users-buttons">
      <Button on:click={displayUsers}>more users</Button>
    </div>
  {:else if users.length > maxDisplayedUsers && users.length <= (currentUser + maxDisplayedUsers)}
    <div class="list-users-buttons">
