<script lang="ts">
    ...
    let currentSolutionsUrl = `/solution?solution_user=${$userID}&solution_offset=1&solution_limit=${maxDisplayed}`;
    ...
    const getSolutions = async () => {
      fetch(`${currentSolutionsUrl}`, {
        ...
        if (response.status === 200) {
          ...
          console.log("You haven't handed in a solution yet :(");
        } else if(response.status === 403){
            alert("Error: " + response.status + " You might not have access to this data (yet).");
            console.log("Admin access to this data only.");
