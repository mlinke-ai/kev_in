...
from backend.lib.core.config import JWT_SECRET, SOLUTION_TABLE, USER_TABLE, UserRole
...
def authorize(
    ...
    if method not in ["GET", "POST", "PUT", "DELETE"]:
        raise ValueError(
            "Argument 'method' should contain one of the following: 'GET', 'POST', 'PUT', 'DELETE'"
        )

    if endpoint not in ["exercise", "user", "solution"]:
        raise ValueError(
            "Argument 'endpoint' should contain one of the following: 'exercise', 'user', 'solution'"
        )

    if (
        method in ["GET", "PUT", "DELETE"]
        and resourceId == None
        and endpoint != "exercise"
    ):
        raise ValueError(
            f"'method' is {method}, 'endpoint' is {endpoint} and no 'recourceId' was provided"
        )

    if method == "PUT" and endpoint == "user" and changeToAdmin == False:
        raise ValueError(
            f"'method' is {method}, 'endpoint' is {endpoint} and no 'changeToAdmin' was provided"
        )
    ...
    if endpoint == "exercise":
        ...
        return not (role == UserRole.User), _authUser(
            role, method, int(user_data["user_id"]), resourceId, changeToAdmin
        )
    elif endpoint == "solution":
        return not (role == UserRole.User), _authSolution(
            role, method, int(user_data["user_id"]), resourceId
        )
...
def _authUser(
    role: UserRole, method: str, userId: int, resourceId: int, changeToAdmin: bool
) -> bool:
    """
    ...
    if method == "GET" or method == "DELETE":
        if role == UserRole.User:
            ...
            query = (
                db_engine.select(user_table)
                .select_from(user_table)
                .where(user_table.c.user_id == resourceId)
            )
            selection = db_engine.session.execute(query)
            try:
                row = selection.fetchone()
        ...
        return (
            True  # this is never reached because user POST does not need authorization
        )
    ...
    if method == "GET" or method == "PUT" or method == "DELETE":
        if role == UserRole.User:
            ...
            solution_table = sqlalchemy.Table(
                SOLUTION_TABLE, db_engine.metadata, autoload=True
            )
            query = (
                db_engine.select(solution_table)
                .select_from(solution_table)
                .where(solution_table.c.user_id == resourceId)
            )
            selection = db_engine.session.execute(query)
            try:
                row = selection.fetchone()
